...
TABLE WITHOUT ID
  week as "📅 周",
  length(rows) as "📝 记录数",
  choice(length(filter(rows, (r) => r.duration AND typeof(r.duration) = "number" AND r.duration > 0)) > 0, 
    sum(filter(rows.duration, (x) => typeof(x) = "number" AND x > 0)), 
    "暂无") as "⏱️ 运动时长(分钟)",
  choice(length(filter(rows, (r) => r.sleep AND typeof(r.sleep) = "number" AND r.sleep > 0)) > 0,
    round(avg(filter(rows.sleep, (x) => typeof(x) = "number" AND x > 0)), 1),
    "暂无") as "😴 平均睡眠(小时)"
FROM "10-Daily"
WHERE date != null AND date >= date(today) - dur(30 days)
...
TABLE WITHOUT ID
  "🏃 健康" as "🏷️ 领域", 
  length(filter(rows, (r) => contains(r.file.content, "<!-- area:health -->") OR (r.duration AND typeof(r.duration) = "number" AND r.duration > 0))) as "📊 活动天数"
FROM "10-Daily"
WHERE date != null AND date >= date(today) - dur(7 days)
GROUP BY true
LIMIT 1
